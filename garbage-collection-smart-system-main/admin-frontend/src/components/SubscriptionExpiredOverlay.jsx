...
export default function SubscriptionExpiredOverlay() {
  ...
  const [subStatus, setSubStatus] = useState(null); // null = still loading

  useEffect(() => {
    api.get("/subscriptions/my-status")
      .then(res => setSubStatus(res.data))
      .catch(() => setSubStatus({ status: 'ERROR' }));
  }, []);

  // Don't render while loading, or when subscription is active / in grace period
  if (!subStatus || subStatus.status !== 'EXPIRED') return null;
          ...
          <div style={{
            ...
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
              <div>
                ...
                <div style={{ fontSize: 15, fontWeight: 700, color: isDark ? "#f1f5f9" : "#0f172a" }}>{subStatus.planName || "—"}</div>
              </div>
              <div style={{ textAlign: "right" }}>
                ...
                <div style={{ fontSize: 15, fontWeight: 700, color: "#f87171" }}>{fmtDate(subStatus.graceEndDate)}</div>
              </div>
            </div>
