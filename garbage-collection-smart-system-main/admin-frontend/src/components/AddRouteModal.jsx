...
export default function AddRouteModal({ isOpen, onClose, onSuccess }) {
  ...
  useEffect(() => {
    if (formData.ward) {
      ...
      // Auto-select all bins for the ward initially
      setFormData(prev => ({
        ...prev,
        stops: bins.map(b => ({ stopName: `${b.locationText} (${b.binCode})` }))
      }));
    } else {
      setFilteredBins([]);
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg border border-gray-100 animate-fade-in-up overflow-hidden max-h-[90vh] flex flex-col">
        ...
        <div className="flex justify-between items-center px-6 py-5 flex-shrink-0"
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h2 className="text-white font-bold text-sm">Add New Route</h2>
            </div>
          </div>
        ...
        <div className="flex justify-end gap-3 px-6 pb-6 pt-4 border-t border-gray-50 flex-shrink-0">
          ...
          <button onClick={handleSubmit} disabled={loading}
            ...
            style={{ background: 'linear-gradient(135deg, #1f9e9a, #16847f)' }}>
            {loading ? <><span className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" /> Creating...</> : 'Create Route'}
          </button>
        </div>
      </div>
    </div>
